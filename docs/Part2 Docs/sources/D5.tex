...
interpreter. No Part-1 contract was modified. One acknowledged
docstring drift survives into the submission: \texttt{runtime\_values.py:137}
describes the \texttt{RtRecipe.steps} field as if it were used for
lazy step rendering, but the interpreter eagerly evaluates step
bodies at instantiation and \texttt{scale} then re-executes them under
the rebound \texttt{servings}. I left the docstring as-is rather than
risk a late-cycle code change that would have rippled through D2/D3
in the final hours; it is filed as a v2 polish item.
